...
def main(args):
    ...
    if args.output_column_tag is None:
        args.output_column_tag = f"{args.response_column}"
    ...
    # Load existing output file if it exists, otherwise start fresh
    if os.path.exists(output_path):
        output_df = pd.read_csv(output_path)
        print(f"Found existing output file: {output_path}")
        
        # Ensure output_df has the same index as df
        if len(output_df) == len(df):
            ...
            print(f"Warning: Output file has different length ({len(output_df)}) than input ({len(df)}). Starting fresh.")
            for col in out_cols:
                df[col] = None
    else:
    ...
    def needs_processing(row):
        return any(pd.isna(row[col]) or row[col] == '' or row[col] == 'ERROR' for col in out_cols)
    
    rows_to_process = df[df.apply(needs_processing, axis=1)]
    total_to_process = len(rows_to_process)
    already_done = len(df) - total_to_process
    
    print(f"Total rows: {len(df)}")
    print(f"Already processed: {already_done}")
    print(f"Remaining to process: {total_to_process}")

    # Process rows one at a time with incremental saving
    save_interval = args.save_interval if hasattr(args, 'save_interval') else 10
    processed_count = 0
    
    for idx in tqdm(rows_to_process.index, desc="Scoring responses"):
        row = df.loc[idx]
        
        # Skip if already processed
        if not needs_processing(row):
            continue
        
        try:
            # Generate scores for this row
            scores = generate_ai_response_for_row(
                row,
                args.prompt_column,
            ...
            for col, score in zip(out_cols, scores):
                df.at[idx, col] = score
            ...
            if processed_count % save_interval == 0:
                ...
                print(f"\nProgress saved: {processed_count}/{total_to_process} rows processed")
                
        except KeyboardInterrupt:
            print("\nInterrupted by user. Saving progress...")
            df.to_csv(output_path, index=False)
            print(f"Saved {processed_count} processed rows to: {output_path}")
            sys.exit(0)
        except Exception as e:
    ...
    # Final save
    df.to_csv(output_path, index=False)
    print(f"\nAll processing complete! Scored CSV saved to: {output_path}")

    if args.baseline:
        b = args.baseline
        print(f'\nComparison to baseline of {b}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_interval", type=int, default=10, help="Save progress every N rows (default: 10)")
